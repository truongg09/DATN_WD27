...
type ReviewByBooking = Record<number, boolean>;
...
const BookingHistory: React.FC = () => {
  ...
  const [reviewedBookings, setReviewedBookings] = useState<ReviewByBooking>({});
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      const reviewEntries = await Promise.all(
        bookingRows.map(async (booking) => {
          try {
            const reviewRes = await fetchReviewByBooking(booking.id);
            return [booking.id, Boolean(reviewRes.data)] as const;
          } catch {
      ...
      setReviewedBookings(Object.fromEntries(reviewEntries));
  ...
  const columns = useMemo<ColumnsType<BookingRow>>(
    () => [
      ...
      },
      {
        ...
        render: (price: number | string) => (
          <strong className="history-price">{formatPrice(price)}</strong>
        ),
      },
      {
        title: "Trạng thái",
      ...
      },
      {
        ...
        render: (_, record) => {
          const payment = payments[record.id];
          const hasReview = reviewedBookings[record.id];
          const canReview = record.status === "checked_out";
          const hasDeposit = Number(payment?.paidAmount || 0) > 0;
    ...
    [cancellingId, payments, reviewedBookings, nowTick],
  );
