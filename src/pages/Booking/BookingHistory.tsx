...
const getHoldRemainingMs = (createdAt?: string) => {
  ...
  return Math.max(
    dayjs(createdAt).add(HOLD_MINUTES, "minute").diff(dayjs()),
    0,
  );
};
...
const BookingHistory: React.FC = () => {
  ...
  const [refundsByBooking, setRefundsByBooking] = useState<
    Record<number, RefundRow>
  >({});
  const [cancelTarget, setCancelTarget] = useState<BookingRow | null>(null);
  const [cancelPreview, setCancelPreview] = useState<RefundPreview | null>(
    null,
  );
  ...
  const [refundBankBin, setRefundBankBin] = useState<string | undefined>(
    undefined,
  );
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      try {
        ...
        setRefundsByBooking(
          Object.fromEntries(
            refundRows.map((refund) => [refund.bookingId, refund]),
          ),
        );
      } catch {
    ...
    try {
      ...
      message.error(
        err.response?.data?.message ||
          "Không thể tải chi tiết đặt phòng để chỉnh sửa",
      );
    } finally {
  ...
  const runAvailabilityCheck = async (opts?: {
    ...
    if (
      ...
      setAvailabilityError(
        "Vui lòng chọn khoảng thời gian hợp lệ (ngày trả phải sau ngày nhận)",
      );
      setAvailabilityResult(null);
      return;
    }
    ...
    try {
      ...
      setAvailabilityError(
        err.response?.data?.message || "Kiểm tra phòng trống thất bại",
      );
      setAvailabilityResult(null);
    } finally {
  ...
  const saveInfoChanges = async () => {
    ...
    try {
      ...
      const diffRoomType =
        editRoomTypeId && Number(editRoomTypeId) !== Number(currentRT);
      ...
      if (status === "checked_in") {
        if (diffCheckIn || diffRoomType) {
          message.error(
            "Đã check-in rồi: không thể đổi ngày nhận hoặc hạng phòng. Chỉ có thể gia hạn ngày trả!",
          );
          setSavingEdit(false);
          return;
        }
        if (extendingOnly) {
          ...
          message.success(
            "Gia hạn thời gian ở thành công! Đang làm mới dữ liệu...",
          );
          setEditModalOpen(false);
          await loadHistory();
          return;
        ...
        message.warning(
          "Vui lòng chọn ngày trả mới lớn hơn ngày trả hiện tại để gia hạn",
        );
        setSavingEdit(false);
        return;
      }
      ...
      message.warning(
        "Trạng thái booking này không cho phép cập nhật thời gian ở.",
      );
    } catch (err: any) {
  ...
  const addServiceToBooking = async () => {
    ...
    try {
      ...
      const serviceName =
        allServices.find((s) => s.id === Number(newServiceId))?.serviceName ||
        "Dịch vụ";
  ...
  const saveEditServiceQty = async () => {
    ...
    try {
      const res = await updateBookingServiceCharge(
        editBookingId,
        Number(editSvcRow.id),
        {
        ...
        content: (
          <div>
            <p>
              Dịch vụ{" "}
              <strong>
                {editSvcRow.serviceName || editSvcRow.name || " "}
              </strong>{" "}
              đã được cập nhật.
              {delta !== 0 && (
                <span>
  ...
  const removeServiceCharge = async (row: any) => {
    ...
    try {
      const res = await deleteBookingServiceCharge(
        editBookingId,
        Number(row.id),
      );
      const result = (res as any).data ?? res;
      ...
      Modal.success({
        ...
        content: (
          <div>
            <p>
              Đã xóa <strong>{row.serviceName || row.name || "dịch vụ"}</strong>{" "}
              khỏi đơn, hoàn lại{" "}
  ...
  const handleConfirmCancel = async () => {
    ...
    if (refundable > 0 && refundMethod === "bank_transfer") {
      ...
      if (!/^\d{4,30}$/.test(refundAccountNumber.replace(/\s+/g, ""))) {
        message.error(
          "Số tài khoản ngân hàng chỉ được bao gồm các chữ số (0-9)",
        );
        return;
      }
    ...
    try {
      ...
      message.error(
        err.response?.data?.message || "Không thể hủy đặt phòng này",
      );
    } finally {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      if (editingReviewId) {
        ...
        message.success(
          wasHidden
            ? "Đã cập nhật đánh giá, đang chờ duyệt lại!"
  ...
  const renderPaymentStatus = (record: BookingRow) => {
    ...
    const holdRemainingMs =
      isUnpaid && !hasDeposit ? getHoldRemainingMs(record.created_at) : 0;
    const isHoldExpired =
      isUnpaid &&
      !hasDeposit &&
      holdRemainingMs <= 0 &&
    ...
    if (!payment) {
      return (
        <div className="history-payment-cell">
          ...
          <span
            className={`history-hold-time ${isHoldExpired ? "expired" : ""}`}
          >
            {isHoldExpired
    ...
    return (
      <div className="history-payment-cell">
        <Tag
          ...
          color={paymentStatusMap[payment.paymentStatus]?.color || "default"}
        >
    ...
    () => [
            ...
            <span className="history-room-line">
              ...
              {record.status === "checked_in" && record.room_number
                ...
                : `${record.room_type_name || "Đặt phòng"}${
                    record.room_quantity && record.room_quantity > 1
                      ? ` (${record.room_quantity} phòng)`
                      : ""
                  }`}
            </span>
          </div>
      ...
      },
      {
        ...
        render: (status: string) => (
          <Tag
            className="history-status-tag"
            color={bookingStatusMap[status]?.color || "default"}
          >
            ...
            <Space className="history-actions" size="small" wrap>
              ...
              {record.status === "checked_out" && (
                <Space size={4} wrap>
                  <Tooltip
                    title={
                      existingReview ? "Xem/Sửa đánh giá" : "Đánh giá phòng"
                    }
                  >
                    <Button
                      type="primary"
                  ...
                  {existingReview?.status &&
                    existingReview.status !== "approved" && (
                      <Tag
                        color={
                          reviewStatusMap[existingReview.status]?.color ||
                          "default"
                        }
                      >
                        {reviewStatusMap[existingReview.status]?.label ||
                          existingReview.status}
                      </Tag>
                    )}
                </Space>
    ...
    <main className="booking-history-page">
      <section className="booking-history-shell">
        <div className="booking-history-hero">
          <div>
            ...
            <p>
              Theo dõi đặt phòng, thanh toán và thao tác hủy phòng của bạn tại
              một nơi.
            </p>
          </div>
      ...
      >
        {cancelPreviewLoading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <strong>
                Lý do hủy phòng <span style={{ color: "#ff4d4f" }}>*</span>
              </strong>
              <Input.TextArea
                rows={3}
                maxLength={500}
            ...
            {cancelPreview.refundableAmount > 0 ? (
                ...
                <Alert
                  ...
                  message={
                    <>
                      Bạn sẽ được hoàn{" "}
                      <strong>
                        {formatPrice(cancelPreview.refundableAmount)}
                      </strong>{" "}
                      ({Math.round(Number(cancelPreview.refundRate) * 100)}% số
                      tiền đã thanh toán) — còn{" "}
                      {cancelPreview.daysBeforeCheckIn} ngày trước nhận phòng.
                    </>
                  }
              ...
              <Alert
                ...
                description={
                  Number(cancelPreview.refundRate) === 0 &&
                  cancelPreview.daysBeforeCheckIn >= 0
        ...
        okButtonProps={{
          disabled:
            ...
            reviewComment.trim().toLowerCase() ===
              (reviewsByBooking[reviewBooking.id]?.comment || "")
                .trim()
                .toLowerCase(),
        }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div>
            <p style={{ marginBottom: 8 }}>
              Ảnh thực tế (không bắt buộc, tối đa {MAX_REVIEW_IMAGES} ảnh):
            </p>
            <Upload
      ...
      <Modal
        title={
          <Space>
            ...
            {editDetail?.status && (
              <Tag
                color={bookingStatusMap[editDetail.status]?.color || "default"}
              >
                {bookingStatusMap[editDetail.status]?.label ||
                  editDetail.status}
              </Tag>
            )}
          </Space>
      ...
      >
        ...
        {!editLoading && !editDetail && (
          <Alert
            type="error"
            message="Không thể tải chi tiết đặt phòng. Vui lòng thử lại."
            showIcon
          />
        )}
        {!editLoading && editDetail && (
                  ...
                  >
                    ...
                    >
                      ...
                      <Descriptions.Item label="Số đêm">
                        {Math.max(
                          dayjs(editDetail.check_out)
                            .startOf("day")
                            .diff(
                              dayjs(editDetail.check_in).startOf("day"),
                              "day",
                            ),
                          0,
                        )}{" "}
                      ...
                      <Descriptions.Item label="Số khách">
                        {editDetail.adults ?? 0} người lớn,{" "}
                        {editDetail.children ?? 0} trẻ em
                      </Descriptions.Item>
                    </Descriptions>
                        ...
                        >
                          <Select
                            ...
                            options={allRoomTypes.map((rt: any) => ({
                              value: rt.id,
                              label: `${rt.typeName} · ${new Intl.NumberFormat(
                                "vi-VN",
                              ).format(Number(rt.defaultPrice || 0))}đ/đêm`,
                            }))}
                          />
                        </Form.Item>
                      </div>
                            ...
                            >
                              <div>
                                ...
                                {editRoomTypeId
                                  ? allRoomTypes.find(
                                      (rt: any) =>
                                        rt.id === Number(editRoomTypeId),
                                    )?.typeName || "—"
                                  : "—"}
                              </div>
                              <div>
                                <strong>Số đêm:</strong>{" "}
                                {availabilityResult.nights} đêm
                              </div>
                              <div>
                              ...
                              <div>
                                <strong>
                                  Tổng tiền ở (chưa bao gồm dịch vụ cũ):
                                </strong>{" "}
                                <span
                                ...
                                >
                                  {new Intl.NumberFormat("vi-VN").format(
                                    Number(
                                      availabilityResult.totalAmount ?? 0,
                                    ) ||
                                      Number(
                                        availabilityResult.stayAmount || 0,
                                      ) +
                                        Number(
                                          typeof availabilityResult.childSurcharge ===
                                            "object"
                                            ? availabilityResult.childSurcharge
                                                ?.amount || 0
                                            : availabilityResult.childSurcharge ||
                                                0,
                                        ),
                  ...
                  >
                    <div>
                      <h4 style={{ margin: "0 0 8px" }}>
                        Dịch vụ đã cộng trong đặt phòng
                      </h4>
                      <Table
                        size="small"
                        rowKey="id"
                    ...
                    <Modal
                      ...
                      onCancel={() =>
                        !editingSvcSaving && setEditSvcModalOpen(false)
                      }
                      okText="Lưu"
                      cancelText="Hủy"
                      width={520}
                      ...
                      <Form layout="vertical">
                          ...
                          <Form.Item label="Chọn dịch vụ" required>
                            <Select
                              ...
                              options={allServices.map((s: Service) => ({
                                value: s.id,
                                label: `${s.serviceName} — ${new Intl.NumberFormat(
                                  "vi-VN",
                                ).format(Number(s.price || 0))}đ`,
                        ...
                        {newServiceId && newServiceQty > 0 && (
                          <Alert
                            type="info"
                            showIcon
                            message={`Thành tiền tạm tính: ${new Intl.NumberFormat(
                              "vi-VN",
                            ).format(
                              Number(
                                allServices.find(
                                  (s) => s.id === Number(newServiceId),
                                )?.price ?? 0,
