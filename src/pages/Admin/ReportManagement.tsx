...
import { ConfigProvider, Segmented, Row, Col, Skeleton, DatePicker, Button } from 'antd';
import { DownloadOutlined, ReloadOutlined, DollarCircleOutlined, CalendarOutlined, UserAddOutlined, ApartmentOutlined } from '@ant-design/icons';
...
type ReportType = 'revenue' | 'booking' | 'customer' | 'room' | 'service';
...
function ReportManagement() {
  ...
  const [customRange, setCustomRange] = useState<{ from: Dayjs; to: Dayjs }>({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (mode === 'custom') {
          url.searchParams.set('from', customRange.from.format('YYYY-MM-DD'));
          url.searchParams.set('to', customRange.to.format('YYYY-MM-DD'));
        }
  ...
  const periodLabel = useMemo(() => {
    ...
    if (mode === 'year') return `Năm ${from.getFullYear()}`;
    if (mode === 'custom') {
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ['KPI', 'Giá trị'],
      ['Tổng doanh thu', formatVND(data.kpis?.revenueTotal ?? 0)],
      ['Tổng số booking', data.kpis?.bookingsTotal ?? 0],
      ['Khách hàng mới', data.kpis?.newCustomers ?? 0],
      ['Tỷ lệ lấp đầy', `${data.kpis?.occupancyRate ?? 0}%`],
      [],
      ['Doanh thu theo thời gian'],
      ['Thời gian', 'Doanh thu'],
      ...(data.revenueSeries.categories.map((cat, i) => [cat, formatVND(data.revenueSeries.data[i])])),
      [],
      ['Số booking theo thời gian'],
      ['Thời gian', 'Số booking'],
      ...(data.bookingSeries.categories.map((cat, i) => [cat, data.bookingSeries.data[i]])),
      [],
      ['Hình thức thanh toán'],
      ['Phương thức', 'Số lượng'],
      ...(data.paymentMethodDonut.labels.map((label, i) => [label, data.paymentMethodDonut.data[i]])),
      [],
      ['Loại phòng được đặt nhiều'],
      ['Loại phòng', 'Số lượng'],
      ...(data.topRoomTypes.labels.map((label, i) => [label, data.topRoomTypes.data[i]]))
    ]
      .map(row => row.join(','))
      .join('\n');
    ...
    link.download = `bao-cao-${mode}-${dayjs().format('YYYY-MM-DD')}.csv`;
    link.click();
  };
      ...
      <div style={{ background: brand.page, minHeight: '100%', padding: 28, borderRadius: 16 }}>
              ...
              <DatePicker.RangePicker
                ...
                onChange={(dates) => {
                  if (dates && dates[0] && dates[1]) {
                    setCustomRange({ from: dates[0], to: dates[1] });
                  }
                }}
                format="DD/MM/YYYY"
        ...
        {reportType === 'revenue' && (
          <>
            ...
            {!loading && error && (
              ...
              >
                <h2 style={{ margin: '0 0 8px', color: brand.textPrimary }}>Không thể tải báo cáo</h2>
        ...
        {reportType === 'booking' && (
          <div style={{ textAlign: 'center', padding: 60 }}>
            <h2 style={{ color: brand.textPrimary, marginBottom: 8 }}>Báo cáo Booking</h2>
            <p style={{ color: brand.textSecondary }}>Tính năng đang phát triển...</p>
          </div>
        )}

        {reportType === 'customer' && (
          <div style={{ textAlign: 'center', padding: 60 }}>
            <h2 style={{ color: brand.textPrimary, marginBottom: 8 }}>Báo cáo Khách hàng</h2>
            <p style={{ color: brand.textSecondary }}>Tính năng đang phát triển...</p>
          </div>
        )}
        ...
        {reportType === 'room' && (
          <div style={{ textAlign: 'center', padding: 60 }}>
            <h2 style={{ color: brand.textPrimary, marginBottom: 8 }}>Báo cáo Phòng</h2>
            <p style={{ color: brand.textSecondary }}>Tính năng đang phát triển...</p>
          </div>
        )}
        ...
        {reportType === 'service' && (
          <div style={{ textAlign: 'center', padding: 60 }}>
            <h2 style={{ color: brand.textPrimary, marginBottom: 8 }}>Báo cáo Dịch vụ</h2>
            <p style={{ color: brand.textSecondary }}>Tính năng đang phát triển...</p>
          </div>
        )}
      </div>
