...
const RoomList: React.FC = () => {
  ...
  const getRoomImage = (type: string, index: number) => {
    ...
    const imgNumber = (index % 4) + 1;
    let folder = 'standard';
    let imgName = `standard${imgNumber}.jpg`;

    if (lowerType.includes('standard')) {
      folder = 'standard';
      imgName = `standard${imgNumber}.jpg`;
    } else if (lowerType.includes('superior')) {
      folder = 'standard';
      // superior dùng chung standard xoay vòng nhưng lệch index để khác ảnh
      imgName = `standard${((index + 2) % 4) + 1}.jpg`;
    } else if (lowerType.includes('deluxe')) {
      folder = 'deluxe';
      const deluxeNumbers = [1, 3, 4];
      const num = deluxeNumbers[index % deluxeNumbers.length];
      imgName = `deluxe${num}.jpg`;
    } else if (lowerType.includes('family')) {
      folder = 'family';
      const familyNumbers = [1, 2, 4];
      const num = familyNumbers[index % familyNumbers.length];
      imgName = `family${num}.jpg`;
    } else if (lowerType.includes('suite') || lowerType.includes('president') || lowerType.includes('luxury')) {
      folder = 'luxury';
      const luxuryNumbers = [1, 2, 3];
      const num = luxuryNumbers[index % luxuryNumbers.length];
      imgName = `luxury${num}.jpg`;
    }

    return new URL(`../../assets/rooms/${folder}/${imgName}`, import.meta.url).href;
  };
                  ...
                  <div key={room.id} className="room-list-card">
                    <div className="room-list-image">
                      <Link to={`/rooms/${room.id}`}>
                        <img src={getRoomImage(room.room_type_name, index)} alt={`Phòng ${room.roomNumber}`} />
                      </Link>
